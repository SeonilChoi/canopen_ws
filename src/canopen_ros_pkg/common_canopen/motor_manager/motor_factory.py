...
class MotorFactory:
    # 클래스 로거 초기화
    logger = get_logger("motor_factory")
    ...
    def create_motor(motor_config):
        ...
        MotorFactory.logger.info(f"Received vendor_type: '{vendor_type}'")  # 디버깅용 출력 유지
        ...
        if vendor_type == "VendorZeroErr":
            eds_path = os.path.join(canopen_motor_dir, 'common_canopen', 'config', 'ZeroErr Driver_V1.5.eds')
            MotorFactory.logger.info(eds_path)
            # 파일 존재 확인
            if not os.path.exists(eds_path):
                MotorFactory.logger.warn(f"경고: EDS 파일을 찾을 수 없습니다: {eds_path}")
                # 대체 경로 시도
                possible_paths = [
                    ...
                    os.path.join('/home/shs/canopen_ws/install/canopen_ros_pkg/lib/python3.8/site-packages/common_canopen/config', 'ZeroErr Driver_V1.5.eds')
                ]
                
                for path in possible_paths:
                    if os.path.exists(path):
                        eds_path = path
                        MotorFactory.logger.info(f"EDS 파일을 찾았습니다: {eds_path}")
                        break
                    else:
            ...
            # 파일 존재 확인
            if not os.path.exists(eds_path):
                MotorFactory.logger.warn(f"경고: DCF 파일을 찾을 수 없습니다: {eds_path}")
                # 대체 경로 시도
                possible_paths = [
                ...
                for path in possible_paths:
                    if os.path.exists(path):
                        eds_path = path
                        MotorFactory.logger.info(f"DCF 파일을 찾았습니다: {eds_path}")
                        break
                else:
