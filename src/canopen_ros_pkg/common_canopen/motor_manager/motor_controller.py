...
class MotorController:
    ...
    """
    def __init__(self, channel='can0', bustype='socketcan', bitrate=1000000, interface=None, node_name="motor_controller"):
        """
        ...
        :param node_name: ROS2 로깅을 위한 노드 이름
        """
        # ROS2 로거 초기화
        ...
        try:
            ...
            self.logger.info(f"CAN bus connected successfully: channel={channel}, bustype={bustype}, bitrate={bitrate}")
    ...
    def all_motors_init_start(self, interval=0.01):
        self.logger.info("Starting motor initialization sequence...")
        ...
        self.logger.info("Motor initialization sequence completed")
    ...
    def reset_all(self):
        ...
        self.logger.info("Reset all motors")
        ...
        try:
            ...
            self.logger.info("NMT state transition to Operational successful")
        except Exception as e:
            self.logger.error(f"Failed to transition NMT state: {e}")
    ...
    def pdo_mapping_all(self):
        ...
        try:
            ...
            self.logger.info('원격 노드 시작 명령을 전송했습니다.')
        except canopen.SdoCommunicationError as e:
            self.logger.error(f'원격 노드 시작 중 오류 발생: {str(e)}')
    ...
    def sync_start(self, interval=0.01):
        self.logger.info(f"Starting sync with interval: {interval}")
        for node_id, motor in self.motors.items():
            motor.set_dt(interval)

        try:
            self.network.sync.start(interval)
            self.logger.info(f"Sync started successfully with interval: {interval}")
        except Exception as e:
    ...
    def set_position(self, node_id, value):
        if node_id in self.motors:
            ...
            self.logger.warn(f"Node {node_id} not found in motors dictionary.")
    ...
    def get_error_code(self, node_id):
        if node_id in self.motors:
            return self.motors[node_id].get_error_code()
        else:
            self.logger.warn(f"Node {node_id} not found in motors dictionary.")

    def get_error_code_all(self):
        for node_id, motor in self.motors.items():
    ...
    def set_position_by_name(self, name, value):
        ...
        if name in self.name_to_id:
            ...
            self.logger.warn(f"Motor with name {name} not found")
    ...
    def disable_all_motors(self, error_motor_id, error_reason):
        ...
        self.logger.error(f"ERROR: 모터 ID {error_motor_id}에서 문제 발생: {error_reason}")
        self.logger.warn("안전을 위해 모든 모터를 비활성화합니다.")
        ...
        try:
            ...
            for node_id, motor in self.motors.items():
                try:
                    ...
                    self.logger.error(f"모터 {node_id} 비활성화 중 오류 발생: {str(e)}")
            ...
            self.logger.info("모든 모터가 비활성화되었습니다.")
        except Exception as e:
            self.logger.error(f"모터 비활성화 중 오류 발생: {str(e)}")
